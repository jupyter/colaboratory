...
var IPython = (function (IPython) {
    ...
    NotificationArea.prototype.init_notification_widgets = function() {
        ...
        $([IPython.events]).on('status_initializing.Kernel', function() {
            knw.set_message('Kernel initializing');
        });

        $([IPython.events]).on('status_loading.Kernel', function(e, data) {
            var progress = '';
            if (data && 'progress' in data) {
                progress = ' ' + data.progress + '%';
            }
            knw.set_message("Kernel loading" + progress);
        });
